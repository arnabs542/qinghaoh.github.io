...
public List<Integer> countSmaller(int[] nums) {
    ...
    // sorts index by the corresponding elements in nums: nums[index[i]]
...
private void merge(int[] nums, int[] index, int low, int high) {
    ...
    // count of right elements that are strictly less than the current left element
    // nums[index[j]] < nums[index[i]]
    int count = 0;
    for (int k = 0; k < tmp.length; k++) {
        if (i <= mid && (j > high || nums[index[i]] <= nums[index[j]])) {
            // all elements strictly less than nums[index[i]] have been counted
            // in the else clause
            // adds the count to the same position in the final result array
            counts[index[i]] += count;
            tmp[k] = index[i++];
        } else {
            if (i <= mid) {  // nums[index[i]] > nums[index[j]]
                count++;
            }
            tmp[k] = index[j++];
        }
